...
if(globalThis.location && !globalThis.location.host.endsWith("leodev.xyz"))
...
export async function uploadGuildAsset(category, guildId, content) {
    const uploadURL = await AXIOS.post(`/guild/${guildId}/assets/${category}`);
    return await AXIOS.post(uploadURL.data, content, {headers: {"content-type": "application/x-octet-stream"}});
}
