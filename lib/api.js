...
const swrOptions = {
    onErrorRetry: (error, key, config, revalidate, { retryCount }) => {
        ...
        if (
            ...
            if (retry_after_header > 0) retry_after = retry_after_header + 1000;
        }
...
export function postOAuthCallback(code, state) {
    return AXIOS.post("/oauth/callback", undefined, {
        params: { code, state },
    });
}
...
export async function postChallengeEmbed(guildId, channelId) {
    return await AXIOS.post(`/guild/${guildId}/challenge-embed`, {
