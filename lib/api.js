...
export let API_ROUTE = "https://cleaner.leodev.cloud/";
...
export function getFilterRules(guildId) {
    return AXIOS.get(`/v1/guild/${guildId}/filterrules`);
}

export function postFilterRule(guildId, event) {
    return AXIOS.post(`/v1/guild/${guildId}/filterrules/${event}`);
}

export function patchFilterRule(guildId, event, index, data) {
    return AXIOS.patch(
        `/v1/guild/${guildId}/filterrules/${event}/${index}`,
        data
    );
}
