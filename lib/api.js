...
export let API_ROUTE = "https://api.cleanerbot.xyz/";
...
AXIOS.interceptors.request.use((config) => {
    ...
    if(url.startsWith("/user/") || url.startsWith("/guild/") || url.startsWith("/challenge") || url.startsWith("/verification") || url.startsWith("/billing")) {
...
export function useSnapshots(guildId) {
...
export async function putWorker(guildId, source) {
    return await AXIOS.put(`/guild/${guildId}/worker`, source);
}

export async function patchConfig(guildId, options) {
    return await AXIOS.patch(`/guild/${guildId}/config`, options);
}

export async function patchEntitlement(guildId, options) {
    return await AXIOS.patch(`/guild/${guildId}/entitlement`, options);
}

/* Billing */

export async function getStripeCheckoutURL(options) {
    return await AXIOS.get(`/billing/stripe/checkout`, { params: options });
}

export async function getStripePortalURL(options) {
    return await AXIOS.get(`/billing/stripe/portal`, { params: options });
}

export async function getCoinbaseChargeURL(options) {
    return await AXIOS.get(`/billing/coinbase/checkout`, { params: options });
}

/* Branding */

export let ASSETS_AXIOS = axios.create({
    baseURL: "https://cdn.cleanerbot.xyz"
});

export async function uploadGuildAsset(category, guildId, formData) {
    const uploadURL = await AXIOS.post(`/guild/${guildId}/branding/assets/${category}`);
    return await AXIOS.post(uploadURL.data, formData, {headers: {"content-type": "multipart/form-data"}});
}

export async function putGuildVanity(guildId, vanity) {
    return await AXIOS.put(`/guild/${guildId}/branding/vanity`, vanity);
}
