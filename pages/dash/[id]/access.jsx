...
export default function DashboardWrapper() {
    ...
    return (
        <>
            <MetaTags
                title="Access | The Cleaner Dashboard"
            />
                    ...
                    panel={<>
                        <Dropdown
                            ...
                            setCurrent={async value => {
                                const success = await doChange(patchConfig(guildId, { access_permissions: value }));
                                if(success)
                                    setConfig({ ...config, access_permissions: value });
                            }}
                            className="w-40"
                        />
                    </>}
...
function IDSelector({ name, none, placeholder, field, guildId, config, setConfig }) {
        ...
        <>
            ...
            <form
                onSubmit={async e => {
                    ...
                    const copy = [...config[field]];
                    if(copy.indexOf(value) >= 0) return toast.error("Duplicate id");
                    copy.push(value);
                    const success = await doChange(patchConfig(guildId, {[field]: copy}));
                    if(!success) return;
                    setConfig({...config, [field]: copy});
