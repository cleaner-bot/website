...
function GuildIcon({ guild }) {
    if (!guild.icon)
        return (
            <QuestionMarkCircleIcon className="w-16 h-16 mx-auto rounded-full bg-gray-810" />
        );
    ...
    return (
        <picture>
            <source
                type="image/webp"
                data-srcset={`${CDN_BASE}.webp?size=128`}
            />
            <img
                data-src={`${CDN_BASE}.png?size=128`}
                alt=""
                className="flex w-16 h-16 mx-auto rounded-full lazyload bg-gray-810"
            />
        </picture>
    );
}
...
export default function Dashboard() {
    ...
    const hasToLogin =
        isError && isError.response && isError.response.status === 401;
                    ...
                    <div className="flex items-center justify-center h-screen">
                        <div>
                            <h1 className="mb-12 text-2xl font-bold text-center">
                                You must login with Discord to gain access to
                                the Dashboard.
                            </h1>
                            <button
                    ...
                    <>
                        ...
                        {response && !isError && (
                            <>
                                ...
                                <p className="text-center text-gray-200">
                                    By default, the dashboard is restricted to
...
function GuildList({ response, query }) {
    ...
    const servers =
        ...
        [...response.data].sort((a, b) =>
            [
                b.is_suspended - a.is_suspended,
                b.is_added - a.is_added,
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            {isLoading
                ...
                : servers
                      ...
                      .map((guild) => <Guild guild={guild} key={guild.id} />)}
        </div>
    );
}
...
function GuildButton({ guild }) {
    ...
    const isComponentAllowed = [
        ...
        "verification",
        "workers",
    ].includes(router.query.component);
    const link =
        `/dash/${guild.id}` +
        (isComponentAllowed ? `/${router.query.component}` : "");
    ...
    return (
        <div className="mx-4 mt-4">
            <button
                className="w-full --btn --btn-4 --btn-success"
                onClick={() => {
                    const url = createOAuthRedirect({
                        bot: true,
                        guild: guild.id,
