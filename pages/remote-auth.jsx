...
export default function RemoteAuth() {
    return (
        <>
            <MetaTags title="Remote authentication" />
            <RemoteAuthWrapper />
        </>
    );
}
    ...
    return (
        <div className="--container">
            ...
            <p className="mb-12 text-center">
                Giving someone this URL or QR-Code will give them{" "}
                <b>full control</b> over The Cleaner&apos;s settings.
            </p>
            {state.stage === 0 && (
                <div className="grid grid-cols-3 gap-4 mx-auto w-96">
                    <Link href="/">
                    ...
                    <button
                        className="--btn --btn-1 --btn-destructive"
                        onClick={async () => {
                            setState({ stage: 1 });
                            let response;
                            try {
            ...
            {state.stage === 1 && (
                <p className="text-center text-red-300">Generating link...</p>
            )}
            {state.stage === 2 && (
                <>
                    <h2 className="text-lg font-semibold text-center">Link:</h2>
                    <p className="text-center">
                        {state.showLink ? (
                            <Link href={makeLinkFromCode(state.code)}>
                    ...
                    {state.showQr ? (
                        ...
                        <div className="flex justify-center mt-4">
                            <button
                                className="w-64 h-64 bg-gray-600 rounded-lg"
                                onClick={() => {
