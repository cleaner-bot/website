...
export default function Challenger({ baseUrl, field, createOAuthRedirect, singleAccount }) {
    ...
    useEffect(async () => {
        ...
        setState({ stage: response.data.is_valid ? 2 : 6, response });
    }, [router]);
        ...
        <div className="flex flex-col items-center justify-center h-screen --container">
            ...
            {state.stage === 2 && <div>
                ...
                <p className="mt-6 text-center text-gray-200">
                    Logged in as <span className="text-blue-300">{state.response.data.user.name}#{state.response.data.user.discriminator}</span>.
                </p>
                <div className="grid gap-4 mt-12">
                    <button className="--btn --btn-primary --btn-3" onClick={async () => {
                        if(state.response.data.captcha_required)
            ...
            {state.stage === 6 && <div>
                ...
                <p className="mt-6 text-center text-gray-200">
                    Are you <span className="text-blue-300">{state.response.data.user.name}#{state.response.data.user.discriminator}</span>?
                </p>
